...
def sendMessageWithInlineKeyboard(chatId, response, messageId = None) :
        inline_keyboard = [
            array(
...
def saveLastUsedBin(userId, bin) :
        # Each user has a unique file named by their ID
        filename = "bincache/last_used_bin_userId.txt"
...
def generateCC(input, quantity = 20) :
    ...
    response .= "𝘽𝙊𝙏 𝙊𝙒𝙉𝙀𝙍 ↯ @spiderslatt"
    
        return response

def generateCCNumber(bin, input) :
        ccNumber = bin 
...
def calculateLuhnCheckDigit(number) :
        sum = 0
...
def generateRandomCVV() :
        return str_pad(rand(1, 999), 3, "0", STR_PAD_LEFT)

def generateRandomCVVWithLuhn(ccNumber) :
...
def parseCCFormat(input) :
        if (strpos(input, '|') !== False) :
                ccInfo = []
                parts = explode('|', input)
                if (count(parts) >= 4) :
                        ccInfo['ccNumber'] = str_replace('x', None, parts[0])
                ...
                return ccInfo
                    ...
                    ccInfo = []
                    ccInfo['ccNumber'] = str_replace('x', None, input)
                    ccInfo['expirationMonth'] = str_pad(rand(1, 12), 2, "0", STR_PAD_LEFT)
                    ccInfo['expirationYear'] = "20" . rand(24, 34) # Modified here
